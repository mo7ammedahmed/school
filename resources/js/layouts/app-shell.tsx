...
function BrandLockup({ compact = false }: { compact?: boolean }) {
    ...
    return (
        <Link href="/dashboard" className="group flex items-center gap-2.5" aria-label={t(locale, 'brand.aether')}>
            <span className="relative flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gradient-to-br from-pine-700 to-pine-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.2),0_2px_6px_rgba(6,40,30,0.35)]">
                <span className="font-display text-[1.05rem] font-semibold leading-none text-white">A</span>
                <span className="absolute -bottom-2 -end-2 size-3 rounded-full bg-gold-300/90" aria-hidden="true" />
            </span>
            {!compact && (
                <span className="flex min-w-0 flex-col leading-none">
                    <span className="truncate text-[0.95rem] font-semibold tracking-[-0.01em] text-foreground">
...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
    ...
    return (
        <div className="flex h-full flex-col">
            ...
            <nav
                className="sidebar-scroll flex-1 space-y-5 overflow-y-auto px-3 py-5"
                aria-label={locale === 'ar' ? 'التنقل الجانبي' : 'Sidebar navigation'}
            >
                {visibleGroups.map((group, gi) => (
                    <div key={gi} className="space-y-0.5">
                        {group.labelKey && (
                            <p className="px-3 pb-1.5 text-[0.625rem] font-semibold uppercase tracking-[0.16em] text-muted-foreground/70">
                                ...
                                return (
                                    <li key={item.key}>
                                        <Link
                                            ...
                                            className={cn(
                                                'group flex items-center gap-3 rounded-lg px-3 py-2 text-[0.875rem] font-medium transition-all duration-150',
                                                active
                                                    ? 'bg-primary/8 text-primary shadow-[inset_0_0_0_1px_rgba(10,92,66,0.06)]'
            ...
            <div className="shrink-0 border-t border-border/70 p-3">
                <div className="flex items-center gap-3 rounded-xl bg-muted/50 p-3">
                    <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-pine-600 to-pine-900 text-sm font-semibold text-white shadow-sm">
    ...
    return (
        <div className="min-h-screen bg-background">
            <div className="flex min-h-screen">
                {/* Desktop sidebar */}
                <aside className="sticky top-0 hidden h-screen w-[17rem] shrink-0 border-e border-border/70 bg-card/70 backdrop-blur-sm md:block">
                    <SidebarContent />
                </aside>
                ...
                {drawerOpen && (
                    <div className="fixed inset-0 z-[60] md:hidden" role="dialog" aria-modal="true">
                        ...
                        />
                        <div className="absolute inset-y-0 start-0 w-[85vw] max-w-[20rem] bg-card shadow-[var(--shadow-panel)]">
                            <button
                                type="button"
                                aria-label="Close menu"
                                onClick={() => setDrawerOpen(false)}
                                className="absolute end-3 top-4 flex size-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground"
                            >
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                    <header className="sticky top-0 z-40 flex h-16 shrink-0 items-center justify-between gap-3 border-b border-border/70 bg-background/85 px-4 backdrop-blur-md md:px-8">
                        <div className="flex min-w-0 items-center gap-3">
                            ...
                            {title && (
                                <h2 className="truncate text-[0.9375rem] font-semibold tracking-[-0.01em] md:hidden">
                                    {title}
                                </h2>
                            )}
                            <span className="hidden items-center gap-2 rounded-full border border-border/70 bg-card px-3 py-1 text-xs text-muted-foreground md:inline-flex">
                                <Sparkles className="size-3 text-gold-500" aria-hidden="true" />
                            ...
                            {canNotify && (
                                ...
                                >
                                    <Link href="/announcements">
                                        ...
                                        <span className="pointer-events-none absolute end-1.5 top-1.5 size-1.5 rounded-full bg-gold-500 ring-2 ring-background" aria-hidden="true" />
                                    </Link>
                                </Button>
                            )}
                    ...
                    <main className="flex-1 px-4 py-6 md:px-8 md:py-8">
                        <div className="mx-auto w-full max-w-[96rem]">
                            {breadcrumbs && breadcrumbs.length > 0 && (
                                <nav
                                    className="mb-5 flex flex-wrap items-center gap-1.5 text-[0.8125rem] text-muted-foreground"
